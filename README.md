...
- Кнопка "Build for free" сприймалась плейрайтом як клікабельна, хоча фактично клік міг не відпрацювати. Тому був створений поллер, який перевіряв зміну урла при натисканні кнопки.
- Сайт динамічно змінює урл після натискання "Build for free" без фізичного переходу (без запита на бек) на сторінку логіна. Тому ловити його довелось через JS (хоча можна було ловити не урл, а елемент сторінки логіна)
- Екран завантаження білдера періодично спливав швидко, тому треба було ставити на паузу виконання скриптів в браузері на вкладці Source, щоб локалізувати його (знайти селектори)
- Неможливість взаємодіяти з елементами білдера, окрім як по координатам пікселів. Так як елементи білдера грузяться в канвасі в ріалтаймі і не відмальовуються в DOM. Відповідно закриття onboarding-алерта я виконав через клік по координатам. Що є робочим рішенням для фіксованих розмірів екрана. Але при його зміні потребуватиме нових координат для кліка. 
