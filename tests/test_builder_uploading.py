def test_builder_uploading(main_page, login_page, stage_builder_page, logout_after_test):
    main_page = main_page()
    main_page.click_build_for_free_button(is_user_logged_in=False)
    login_page = login_page(is_open=False)
    login_page.login()
